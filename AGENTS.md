...
**AI Provider layer** (`src/ai-provider.js`): Unified `callAI()` and `callVisionAI()` supporting 8 providers (Ollama, Codex, OpenAI, DeepSeek, Qwen, Gemini, NVIDIA, Groq). Config in `src/config.js`.
