...
function _qaIsQuotaError(err) {
    const status = err.response?.status;
    if (status === 403 || status === 404) return 'exhausted';
    if (status === 429) return 'rate_limited';
    return null;
}
    ...
    switch (nicheId) {
        ...
        case 'news.military':
            return [
                ...
                '  - Footage from Telegram/VK: lower quality, Arabic/Russian/Cyrillic captions, field camera work = NORMAL for this source.',
                '  - Baked-in Arabic or Cyrillic subtitles from Telegram = TYPE A caption bars — apply caption_shift or scale fix, NOT rejection.',
                '  - For replacements: prefer youtube, then telegram/reddit for conflict zone footage.',
            ].join('\n');

        case 'news.politics':
            return [
                '  - Press conferences, parliament/congress sessions, protest marches, diplomatic events, government buildings = all expected.',
                '  - Telegram/VK footage may include press briefings and government announcements — valid for political news.',
                '  - Political leaders in news context = FINE (not a presenter face).',
                '  - For replacements: youtube first, telegram for specific government/regional events.',
            ].join('\n');
        ...
        case 'news.celebrity':
            return [
                ...
                '  - For replacements: youtube or bing/googleScrape using celebrity name + event.',
            ].join('\n');
        ...
        case 'news.economy':
            return [
                ...
                '  - For replacements: youtube for corporate/market footage, pexels for generic business B-roll.',
            ].join('\n');
...
function _buildReplacementSourceGuidance(nicheId) {
    ...
    if (isExplainer) {
        return [
            ...
            '  - Photos / news images → bing or googleScrape',
        ].join('\n');
    }
    ...
    if (isNews) {
        ...
        return [
            ...
            '  - Financial / economic visuals → youtube or pexels',
            '  - Photos / news images → bing or googleScrape',
            '  - Generic B-roll fallback → pexels or pixabay',
        ].filter(Boolean).join('\n');
    }

    return [
        '  Available: pexels | pixabay | youtube | telegram | reddit | vkVideo | bing | googleScrape | unsplash',
        '  - Real news events / political / military footage → youtube or telegram',
        '  - Historical archive footage → youtube or pexels',
        '  - Generic B-roll → pexels or pixabay',
        '  - News images / web photos → bing or googleScrape',
    ].join('\n');
}
...
- The pipeline: AI Director analyzes the script → AI Visual Planner assigns footage keywords + MG types per scene → footage is downloaded from stock providers (Pexels, Pixabay, etc.) → Motion Graphics are composited on top.
...
SCENE UNDER REVIEW:
  ...
  Source    : ${sceneInfo.sourceHint || 'stock'}${sceneInfo.sourceHint === 'telegram' ? ' — expect raw news/war footage, lower production quality is normal' : sceneInfo.sourceHint === 'reddit' ? ' — expect broadcast captures or drone footage' : sceneInfo.sourceHint === 'youtube' ? ' — expect documentary or real-world footage' : sceneInfo.sourceHint === 'web-image' ? ' — this is a web image (photo), not video' : ' — expect cinematic stock B-roll'}
...
REPLACEMENT_DIAGNOSIS: <Only fill if FIX_STRATEGY is "flag". Diagnose WHY the footage failed in one sentence. Was it:
  ...
  - Wrong source (stock footage has no real events; youtube/telegram needed for actual news footage)?
...
function _parseResponse(text, sceneIndex) {
    ...
    for (const line of text.split('\n').map(l => l.trim()).filter(Boolean)) {
        ...
        if (lower.startsWith('scene_summary:')) {
            ...
            if (!val.startsWith('none')) result.replacementSource = val.trim();
...
async function analyzeSceneClip(clipPath, sceneInfo, context) {
    ...
    try {
        ...
        if (result.issues.includes('center_overlay') && !result.replacementKeyword && sceneInfo.keyword) {
            ...
            result.replacementSource  = result.replacementSource || sceneInfo.sourceHint || 'pexels';
