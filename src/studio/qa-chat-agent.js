...
const { getNiche, NICHES, NICHE_PRESETS, getNicheIds } = require('../data/niches');
...
- Pipeline: AI Director → AI Visual Planner → footage download → Motion Graphics compositing → HyperFrames render (HTML/GSAP captured frame-by-frame).
...
- Provider access is niche-specific. Use the resolved niche data below for exact exclusions and footage priority.
...
- Stock providers (Storyblocks): licensed B-roll, clean cinematic footage, and map visuals.
...
FIX_STRATEGY: scale (crop+zoom) | floating (switch to float card) | caption_shift (crop caption bar) | flag (replace footage)`);
    ...
    parts.push(`
CRITIQUE DISCIPLINE:
- Anchor every issue to the EXACT scene (scene id / index / timestamp) and the specific plan field or on-screen element — never a vague "the video feels off".
- After you find one issue, sweep the SAME CLASS across the rest of the video (if scene 3's caption is desynced, check every caption) and report the whole set, not just the first.
- If something looks wrong but you cannot name a concrete fix, label it an INVESTIGATION (something to look at), not a blocking defect. Reserve "reject" for concrete, verifiable problems.`);
