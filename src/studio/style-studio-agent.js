...
- sourceHint: stock | youtube | web-image | reddit (REQUIRED — pick source FIRST, then write the keyword for THAT source)
...
Pick the source FIRST based on what the scene needs, THEN write the keyword in the style that source expects. A keyword that works on Storyblocks will fail on YouTube, and vice versa.

1) sourceHint: stock  (Storyblocks)
...
4) sourceHint: reddit
   - Audience: community-uploaded broadcast/drone/dashcam footage for scenes where YouTube is not ideal.
...
function _normalizeExtractedProfile(profile, session = null) {
    ...
    if (Array.isArray(rawMG)) {
        let MG_REGISTRY = {};
        try { MG_REGISTRY = require('../render/mg-registry').MG_REGISTRY || {}; } catch (_) {}
...
async function _runScriptAnalysis(session) {
    try {
        ...
        let nicheIds = [];
        try { nicheIds = require('../data/niches').getNicheIds(); } catch (_) {}
...
function _buildNicheRulesBlock(nicheId) {
    ...
    let niches;
    try { niches = require('../data/niches'); } catch (_) { return ''; }
    let niche;
...
async function _handleSceneSplitRequest(session, originalMessage) {
    const fs = require('fs');
    const path = require('path');
    const director = require('../agents/ai-director');
...
async function _handleSaveScenePlan(session, originalMessage) {
    ...
    const parseSceneLine = (sm) => {
        ...
        for (const part of parts) {
            ...
            else if (k === 'sourcehint') scene.sourceHint = sanitizeSourceHint(v.toLowerCase(), 'youtube');
