...
function _ensureLoaded() {
    ...
    const fm = require('./footage-manager');
 ...
 * @param {string} params.sourceHint        - qaReplacementSource (pexels/youtube/telegram/…)
 ...
 */
async function replaceSceneMedia({ mediaFile, keyword, sourceHint, mediaType = 'video', sceneDuration = 8, scriptContext = {}, onProgress }) {
    _ensureLoaded();
...
async function _searchMultipleArticles(articleImage, keyword, subtext) {
    ...
    const SKIP_DOMAINS = require('./article-image').__SKIP_DOMAINS || [];
