...
function _uniqueCanonicalNames(names, entities) {
    const out = [];
    const seen = new Set();
    for (const raw of Array.isArray(names) ? names : []) {
        const canon = _canonicalizeName(raw, entities);
        const key = _subjectKey(canon);
...
const SUBJECT_MATCH_STOPWORDS = new Set([
    ...
    'that', 'this', 'these', 'those', 'route', 'routes', 'gateway', 'map',
    'label', 'region', 'area', 'place', 'zone', 'zones'
]);

function _subjectTokens(name) {
    return _subjectKey(name)
        .split(/\s+/)
        .filter(t => t.length >= 3 && !SUBJECT_MATCH_STOPWORDS.has(t));
}

function _textContainsSubject(text, subject) {
    const haystack = ` ${_subjectKey(text)} `;
    if (!haystack.trim()) return false;
    const tokens = _subjectTokens(subject);
    if (tokens.length === 0) return false;
    if (tokens.every(t => haystack.includes(` ${t} `))) return true;

    // Transcripts often mangle "Bab" as "Bob" or drop the small connector
    // words. A distinctive long token such as "mandeb", "hormuz", "djibouti",
    // or "eritrea" is enough to prove the subject belongs to this scene window.
    const distinctive = tokens.filter(t => (
        t.length >= 5
        && !['strait', 'canal', 'gulf', 'ocean', 'channel'].includes(t)
    ));
    return distinctive.some(t => haystack.includes(` ${t} `));
}

function _sceneWindowText(scene, sceneByIndex, radius = 2) {
    const idx = Number(scene?.index);
    const parts = [];
    if (Number.isFinite(idx) && sceneByIndex instanceof Map) {
        for (let i = idx - radius; i <= idx + radius; i++) {
            const neighbor = sceneByIndex.get(i);
            if (neighbor?.text) parts.push(neighbor.text);
        }
    }
    if (scene?.visualIntent) parts.push(scene.visualIntent);
    if (scene?.keyword) parts.push(scene.keyword);
    if (scene?.webQuery) parts.push(scene.webQuery);
    return parts.join(' ');
}

function _subjectsOverlap(a, b) {
    const ak = _subjectKey(a);
    const bk = _subjectKey(b);
    return !!ak && !!bk && (ak === bk || ak.includes(bk) || bk.includes(ak));
}

function _variantForMode(mode) {
    return mode === 'region' ? 'regionHighlight' : mode;
}

function _buildMapChartPayload(names) {
    if (!Array.isArray(names) || names.length === 0) return null;
    return `mapChart: ${names.map(n => `${n}: label`).join(', ')}`;
}
...
function _hasRouteNarrativeIntent(scene, disposition, names, entityTypes) {
    if (!Array.isArray(names) || names.length < 2) return false;
    if (_isTradeCorridorBetweenRegions(scene, disposition, names, entityTypes)) return true;

    const verb = String(disposition?.signals?.spatialVerb || '').toLowerCase();
    const text = `${scene?.text || ''} ${scene?.visualIntent || ''}`.toLowerCase();
    const joined = `${verb} ${text}`;

    if (/\bfrom\s+[a-z0-9][a-z0-9\s,'-]{1,80}\s+(?:to|toward|towards)\b/.test(joined)) return true;
    if (/\b(?:heads?|headed|heading|travell?(?:ed|ing)?|sail(?:ed|ing)?|move(?:d|s|ing)?|push(?:ed|ing)?|advanc(?:e|ed|ing)|flow(?:s|ed|ing)?|rerout(?:e|ed|es|ing)|bypass(?:ed|es|ing)?|divert(?:ed|s|ing)?)\s+(?:from|to|toward|towards|through|across|along|around|into|out of)\b/.test(joined)) return true;
    if (/\b(?:route|routes|corridor|corridors|path|paths|passage|shipping\s+lane|sea\s+lane)\b/.test(text)
        && /\b(?:from|to|between|via|through|around|bypass|detour|alternative|alternate|backup)\b/.test(text)) {
        return true;
    }
...
function compileMapScenes(scenes, scriptContext, dispositions) {
    ...
    let policy;
    try {
        const { getNicheMapPolicy } = require('../data/niches');
    ...
    const sceneByIndex = new Map(
        scenes
            .filter(s => s && Number.isFinite(Number(s.index)))
            .map(s => [Number(s.index), s])
    );
    ...
    for (const scene of scenes) {
        ...
        // ── Resolve subjects ──
        // Disposition matchedPlaces are the scene-local anchor. VP payload
        // subjects are kept only when they also belong to the nearby narration.
        const localNames = _uniqueCanonicalNames(disposition?.signals?.matchedPlaces || [], entities);
        const payloadNames = _uniqueCanonicalNames(_extractSubjectsFromPayload(payload), entities);
        let rawNames = [];
        if (payloadNames.length) {
            const contextText = _sceneWindowText(scene, sceneByIndex, 2);
            const sceneOnlyText = `${scene.text || ''} ${scene.visualIntent || ''}`;
            const startTime = Number(scene.startTime);
            const isFirstHookMap = /first hook/i.test(String(disposition?.reason || ''))
                || Number(scene.index) === 0
                || (Number.isFinite(startTime) && startTime <= 1.5);
            const keptPayload = [];
            const droppedPayload = [];
            for (const name of payloadNames) {
                const matchesLocal = localNames.some(local => _subjectsOverlap(name, local));
                const appearsInContext = _textContainsSubject(isFirstHookMap ? sceneOnlyText : contextText, name);
                if (!localNames.length || matchesLocal || appearsInContext) {
                    keptPayload.push(name);
                } else {
                    droppedPayload.push(name);
                }
            }
            if (droppedPayload.length > 0 && localNames.length > 0) {
                console.log(
                    `      [Map Guard] Scene ${scene.index} dropped off-scene payload subject(s): ` +
                    droppedPayload.map(n => `"${n}"`).join(', ') +
                    ` (local anchor: ${localNames.map(n => `"${n}"`).join(', ')})`
                );
            }
            rawNames = _uniqueCanonicalNames(
                isFirstHookMap && localNames.length
                    ? [...localNames, ...keptPayload]
                    : [...keptPayload, ...localNames],
                entities
            );
        } else {
            rawNames = localNames;
        ...
        if (acceptedNames.length === 1 && (mapMode === 'route' || mapMode === 'comparison')) {
            const priorMode = mapMode;
            mapMode = 'locator';
            scene.mapVariant = _variantForMode(mapMode);
            console.log(`      [Map Guard] Scene ${scene.index} downgraded ${priorMode}->locator (only one local subject after guard)`);
        }

        if (acceptedNames.length >= 2 && mapMode === 'route' && !_hasRouteNarrativeIntent(scene, disposition, acceptedNames, entityTypes)) {
            mapMode = 'region';
            scene.mapVariant = _variantForMode(mapMode);
            console.log(`      [Map Guard] Scene ${scene.index} downgraded route->region (no journey/route language; ${acceptedNames.length} places should be held together)`);
        }
        ...
        const normalizedPayload = _buildMapChartPayload(capped);
        if (normalizedPayload && scene.fullscreenMG !== normalizedPayload) {
            scene.fullscreenMG = normalizedPayload;
        }
        scene.mapVariant = _variantForMode(mapMode);
