...
function _buildItemsFromMatches(matches, scenes, sectionTitles, scriptContext) {
    ...
    for (const m of matches) {
        ...
        // Match the section title by its leading number (e.g. "#9 D.W. Griffith") so the
        // title lines up with the actual itemNumber. Positional indexing was off-by-one
        // because sectionTitles includes the intro section but matches start at item 1.
        const titleByNumber = sectionTitles.find(t => {
            const lead = String(t).match(/^[#\s]*(\d+)\b/);
            return lead && Number(lead[1]) === m.itemNumber;
        });
        items.push({
            itemNumber: m.itemNumber,
            displayLabel: m.displayLabel,
            title: titleByNumber || sectionTitles[items.length] || '',
